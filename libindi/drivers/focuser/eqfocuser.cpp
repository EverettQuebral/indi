/*
  EQFOCUSER
  Copyright (c) 2016 Everett Quebral
  All Rights Reserved.
...
std::unique_ptr<EQFOCUSER> nstep(new EQFOCUSER());
...
bool EQFOCUSER::command(const char *request, char *response, int count) {
...
bool EQFOCUSER::Connect() {
  ...
  bool rc;
  if (isSimulation()) {
    IDMessage(getDeviceName(), "EQFOCUSER simulation is connected.");
    SetTimer(POLLMS);
    return true;
  } else {
...
bool EQFOCUSER::updateProperties() {
  if (isConnected()) {
    command(":CC1", NULL, 0);
    if (command(":RP", buf, 7)) {
...
bool EQFOCUSER::ISNewSwitch (const char *dev, const char *name, ISState *states, char *names[], int n) {
...
bool EQFOCUSER::ISNewNumber (const char *dev, const char *name, double values[], char *names[], int n) {
  if(strcmp(dev, getDeviceName()) == 0) {
...
IPState EQFOCUSER::MoveRelFocuser(FocusDirection dir, unsigned int ticks) {
...
bool EQFOCUSER::SetFocuserSpeed(int speed) {
...
bool EQFOCUSER::saveConfigItems(FILE *fp) {
